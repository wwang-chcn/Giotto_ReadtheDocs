...
- **Use of symbols**
    - Do not use any non-ASCII characters (unless as unicode escape sequences, with alternatives for non-UTF-8 encoding systems)
...
By default, these functions retrieve the information as the S4 subobject. This is a measure to prevent loss of the additional information that may be carried in the other slots of the S4 subobjects. Getter returning as the wrapped S3 is also possible, often through the `output` param or a `return_x` boolean param.
...
### Setting Giotto behavior
Giotto's default behaviors are usually set by using the giotto instructions which are created either through `createGiottoInstructions()` by the user or generated automatically during object creation. These instructions are then appended into the giotto object in the `@instructions` slot. As a rule of thumb, any behaviors that are **NOT** session or system specific should be controlled through the giotto instructions.  
  
Notable examples of the aforementioned session or system-specific exceptions are `'giotto.plot_count'` or `'giotto.unicode'` which live in the R options.
...
### Printout usage
- **verbose**
    - Any functions with printouts should generally have a `verbose` param that is set to `TRUE` by default, where `TRUE` displays the printouts and `FALSE` suppresses them.    
- **wrapping text**
    Giotto includes functions in `utilities.R` for text wrapping.
    - `wrap_txt()` a general purpose function to wrap text to the console window before sending it to a printout function.  
        - ex: `cat(wrap_txt('hello_world'))` or `stop(wrap_txt('my_favorite_error'))`  
    - `wrap_msg()` shorthand for `message(wrap_txt())`  
    
- **styling rules**
    - Use of `message()`-styled printouts are recommended for almost all circumstances for any diagnostic printouts.  
    - `cat()`-style printouts should be reserved for printouts that the user has initiates (for example `show()` functions)  
    - `warning()` usage should be sparingly used and reserved for potential issues that the user should be made aware of about the function operation that or output limitations that are not likely to be vital.  
    - `stop()` errors should be used with most caution, as they stop function execution. These should provide a clear description of what the error is.

$~$

### TRUE FALSE testing
`isTRUE()` should be used when testing if a boolean value is TRUE to aid with code
readability. `isTRUE()` should be used even when testing if value is FALSE by negating
with !.
```{r, eval=FALSE}
a = TRUE
...
#### data.table usage
data.table's syntax does not play too well with R cmd check. Here are some guidelines for working with data.table in ways that will not trigger notes during the package check:

1. When referencing columns as variables, set them to NULL earlier in the function so that the check does not complain `no visible binding for global variable 'foo'`
```{r}
Species = NULL
...
2. When supplying a vector of colnames to subset, it is common to use the `..foo` notation but this will return the note `no visible binding for global variable ‘..foo’` during package check. Instead use the older `with = FALSE` notation.  
Trying to set `..foo = NULL` to avoid the error will instead get a warning from data.table

**Preferred:**  
```{r, eval=FALSE}
foo = c('Species', 'Sepal.Length')
x[, foo, with = FALSE]
```

**Works, but has issues with pkg check:**  
```{r, eval=FALSE}
x[, ..foo]
```
...
```
```{r, eval=FALSE}
# Warning: Both 'foo' and '..foo' exist in calling scope. Please remove the '..foo' variable in calling scope for clarity.
```

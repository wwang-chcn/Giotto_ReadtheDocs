...
Operators Giotto S4 internal data.tables to extract
 or replace parts.
...
`value`     |     value(s) to set This is normally matched to the colnames of the data.table object within the S4.
`i, j`     |     indices specifying elements to extract or replace. Indices are numeric or character vectors or empty

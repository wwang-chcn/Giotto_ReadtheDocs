...
`optional`     |     whether the package is optional. `stop()` is used if TRUE and only `message()` will be sent if FALSE.
`custom_msg`     |     custom message to be sent instead of default error or message

...
`all_spat_units`     |     apply features to remove filtering results from current spatial unit/feature type combination across ALL spatial units (default = TRUE)
`all_feat_types`     |     apply cells to remove filtering results from current spatial unit/feature type combination across ALL feature types (default = TRUE)
`poly_info`     |     polygon information to use
`tag_cells`     |     tag filtered cells in metadata vs. remove cells
`tag_cell_name`     |     column name for tagged cells in metadata
`tag_feats`     |     tag features in metadata vs. remove features
`tag_feats_name`     |     column name for tagged features in metadata
`verbose`     |     verbose

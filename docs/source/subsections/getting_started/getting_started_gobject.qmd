...
## Minimum requirements for a Giotto Object
 ...
 Here, creating a `giotto` object with the minimum requirements is shown in two examples.
...
`createGiottoInstruction()` is used to create the instructions that are provided to `createGiottoObject()`.
The `instructions()` function can then be used to view, set, or modify one or more of these instructions after they have been added to a `giotto` object.
